...
namespace ScribblePlatformer3
{
    class Player : AnimatedSprite
    {
        ...
        private void GetInput()
        {
            ...
            isJumping = gamePadState.IsButtonDown(JumpButton) || keyboardState.IsKeyDown(Keys.Space) ||
                        keyboardState.IsKeyDown(Keys.Up) || keyboardState.IsKeyDown(Keys.W);

            if (isJumping && currentAnim != "Jump" && currentAnim != "Walking")
            {
                SpriteAnimations[currentAnim].Stop();
                currentAnim = "Jump";
                SpriteAnimations[currentAnim].ResetPlay();
